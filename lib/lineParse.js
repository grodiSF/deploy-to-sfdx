		...
		const rl = readline.createInterface({
			...
			if (msgJSON.whitelisted){
				...
				if(typeof msgJSON.accessToken!='undefined' && msgJSON.accessToken!='')
				{
					...
					if(parsedLines[0]!='sfdx force:config:set instanceUrl='+msgJSON.instanceUrl+' --global'){
						parsedLines.unshift('sfdx force:config:set instanceUrl='+msgJSON.instanceUrl+' --global');
					}
				}
