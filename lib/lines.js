...
if (process.env.LOCAL_ONLY_KEY_PATH){
	...
	keypath = '/app/tmp/server.key';
		...
		//before running the lines, import the org details with auth
		if(typeof this.msgJSON.SOusername!='undefined'){
			let orgdetails=await redis.get(this.msgJSON.SOusername);
			logger.debug("ORGDETAILS:"+this.msgJSON.SOusername+" "+orgdetails);
			if(typeof orgdetails!='undefined'){
				
				orgdetails=JSON.parse(orgdetails);
				if (process.env.DXLOGINURL)
				{   
			/*		fs.writeFileSync(process.env.LOCAL_ONLY_AUTHURL_PATH+this.msgJSON.deployId+'.txt', orgdetails.sfdxAuthUrl, 'utf8');
					this.lines.unshift(`cd tmp;cd ${this.msgJSON.deployId};sfdx force:auth:sfdxurl:store -f `+process.env.LOCAL_ONLY_AUTHURL_PATH+this.msgJSON.deployId+'.txt');
				}
				else {*/
			...
			try {
				...
				if (lineResult.stdout) {
					if(localLine.includes('sfdx force:org:display') && localLine.includes('--verbose')){
						...
						console.log("writing in redis cache:"+r.result.username,JSON.stringify(r.result));
		...
		if (process.env.DXLOGINURL && orgdetails)exec(`cd tmp;rm  /app/tmp/${this.msgJSON.deployId}.txt`);
	};
};
